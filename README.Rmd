...
For other packages or situations, you can use `include_meta()` for &#x2139; formats to explicitly declare the meta tags as an html dependency or use `write_meta()` for &#x274C; formats to save the `<meta>` tags to an `.html` file that can be included via `includes: in_header`.

````markdown
```{r pkgdown-meta, echo = FALSE}`r ''`
meta() %>% 
...
meta() %>%
  ...
  meta_social(
    title = "{metathis}",
    url = "https://pkg.garrickadenbuie.com/metathis",
    image = "https://https://github.com/gadenbuie/metathis/raw/master/man/figures/card.png",
