...
[`meta_social()`](http://pkg.garrickadenbuie.com/metathis/reference/meta_social.md)
...
to explicitly declare the meta tags as an html dependency or use
[`write_meta()`](https://pkg.garrickadenbuie.com/metathis/dev/reference/write_meta.md)
to save the `<meta>` tags to an `.html` file that can be included via
`includes: in_header`. (In blogdown, consult your blogdown/hugo theme
for the correct inclusion method.)
...
To use `metathis` in Shiny apps, simply call
[`meta()`](https://pkg.garrickadenbuie.com/metathis/dev/reference/meta.md)
and related tags anywhere inside your page UI, for example inside
`fluidPage()`.
...
slides, add
[`meta()`](https://pkg.garrickadenbuie.com/metathis/dev/reference/meta.md)
and related tags in a chunk anywhere in your slide’s source `.Rmd` file.
This example is from a [presentation on the drake
package](https://pkg.garrickadenbuie.com/drake-intro/).
